...
struct in6_addr ipv4_dst_to_ipv6_dst(uint32_t destination) {
  ...
  //zi ji jia..
  logmsg(ANDROID_LOG_FATAL,"ipv4_dst_to_ipv6_dst");
  const char *plat_subnet="2001:778:0:ffff:64::";
  logmsg(ANDROID_LOG_FATAL,"ipv4 destination:   %x",zhuanhuan(destination));

  destinationsz[0] = destination & 0xff;
  destinationsz[1] = destination >> 8 & 0xff;
  destinationsz[2] = destination >> 16 & 0xff;
  destinationsz[3] = destination >> 24 & 0xff;  

  int i,flag=-1;
  uint32_t destinationzh=zhuanhuan(destination);
  for(i=0;i<length;i++){
    if((daddr4[i]<destinationzh) && (destinationzh<daddr4gb[i])){
      // plat_subnet=ch6[i];
      flag=i;
    }
  }  
  
  //plat_subnet="2001:778:0:ffff:64::";//96
  //plat_subnet="2001:67c:2b0:db32:0:1::";//96
  //plat_subnet="2001:67c:27e4:641::";//64
  //plat_subnet="2001:67c:27e4:11::";//64  
  //plat_subnet="2001:67c:27e4:64::";//64  
  //zi ji jia..
  if(flag!=-1){
    plat_subnet=ch6[flag];
    logmsg(ANDROID_LOG_FATAL,"mysql plat_subnet:  %s",plat_subnet);
    int status = inet_pton(AF_INET6, plat_subnet, &Global_Clatd_Config.plat_subnet);
    if(status <= 0) {
      logmsg(ANDROID_LOG_FATAL,"inet_pton fail");
    }else{
      //logmsg(ANDROID_LOG_FATAL,"inet_pton success");
    }
    // assumes a /96 plat subnet
    v6_destination = Global_Clatd_Config.plat_subnet;
    if(strcmp(fix6[flag],"96")==0){
      for(i=12;i<16;i++){
        v6_destination.s6_addr[i]=destinationsz[i-12];
      }
    }else if(strcmp(fix6[flag],"64")==0){
      //for(i=9;i<13;i++){
      //  v6_destination.s6_addr[i]=destinationsz[i-9];
      //}
      for(i=12;i<16;i++){
        v6_destination.s6_addr[i]=destinationsz[i-12];
      }
    }else if(strcmp(fix6[flag],"56")==0){
      //v6_destination.s6_addr[7]=destinationsz[0];
      //v6_destination.s6_addr[9]=destinationsz[1];
      //v6_destination.s6_addr[10]=destinationsz[2];
      //v6_destination.s6_addr[11]=destinationsz[3];
      for(i=12;i<16;i++){
        v6_destination.s6_addr[i]=destinationsz[i-12];
      }
    }else if(strcmp(fix6[flag],"48")==0){
      //v6_destination.s6_addr[6]=destinationsz[0];
      //v6_destination.s6_addr[7]=destinationsz[1];
      //v6_destination.s6_addr[9]=destinationsz[2];
      //v6_destination.s6_addr[10]=destinationsz[3];
      for(i=12;i<16;i++){
        v6_destination.s6_addr[i]=destinationsz[i-12];
      }
    }else if(strcmp(fix6[flag],"40")==0){
      //v6_destination.s6_addr[5]=destinationsz[0];
      //v6_destination.s6_addr[6]=destinationsz[1];
      //v6_destination.s6_addr[7]=destinationsz[2];
      //v6_destination.s6_addr[9]=destinationsz[3];
      for(i=12;i<16;i++){
        v6_destination.s6_addr[i]=destinationsz[i-12];
      }
    }else if(strcmp(fix6[flag],"32")==0){
      //for(i=4;i<8;i++){
      //  v6_destination.s6_addr[i]=destinationsz[i-4];
      //}
      for(i=12;i<16;i++){
        v6_destination.s6_addr[i]=destinationsz[i-12];
      }
    ...
    logmsg(ANDROID_LOG_FATAL,"moren plat_subnet:  %s",plat_subnet);
    int status = inet_pton(AF_INET6, plat_subnet, &Global_Clatd_Config.plat_subnet);
    if(status <= 0) {
      logmsg(ANDROID_LOG_FATAL,"inet_pton fail");
    }else{
  ...
  logmsg(ANDROID_LOG_FATAL,"ipv6 destination:   %x:%x:%x:%x:%x:%x:%x:%x",zhuanhuan16(v6_destination.s6_addr16[0]),zhuanhuan16(v6_destination.s6_addr16[1]),zhuanhuan16(v6_destination.s6_addr16[2]),zhuanhuan16(v6_destination.s6_addr16[3]),zhuanhuan16(v6_destination.s6_addr16[4]),zhuanhuan16(v6_destination.s6_addr16[5]),zhuanhuan16(v6_destination.s6_addr16[6]),zhuanhuan16(v6_destination.s6_addr16[7]));
