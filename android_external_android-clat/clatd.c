...
void configure_interface(const char *uplink_interface, const char *plat_prefix, struct tun_data *tunnel) {
  ...
  if(!read_config("/data/misc/clatd.conf", uplink_interface, plat_prefix)) {
...
void packet_handler(const struct tun_data *tunnel, struct tun_pi *tun_header, const char *packet, size_t packetsize) {
  ...
  printf("tun_header->proto == ETH_P_IPV6\n");
...
void duwenjian(){
  FILE *fp; 
  logmsg(ANDROID_LOG_FATAL, "duwenjian");
  if((fp=fopen("/data/data/edu.bupt.Clat/files/abc.conf","r"))==NULL) { 
    logmsg(ANDROID_LOG_FATAL, "Cannot open file");
  } 

  fscanf(fp,"%d",&length);
  logmsg(ANDROID_LOG_FATAL, "%d",length);
  char chaddr[50],chqz[5];
  ch4=(char (*)[50])malloc(length*50*sizeof(char));
  ch6=(char (*)[50])malloc(length*50*sizeof(char));
  fix4=(char (*)[50])malloc(length*50*sizeof(char));
  fix6=(char (*)[50])malloc(length*50*sizeof(char)); 
  daddr4 = (uint32_t*)malloc(length * sizeof(uint32_t));
  daddr4gb = (uint32_t*)malloc(length * sizeof(uint32_t));
  int i;
  for(i=0;i<length;i++){
    //ipv6 di zhi
    fscanf(fp,"%s",chaddr);
    strncpy(ch6[i], chaddr, 39);
    //ipv6 qian zhui chang du
    fscanf(fp,"%s",chaddr);
    strncpy(fix6[i], chaddr, 5);
    //ipv4 di zhi
    fscanf(fp,"%s",chaddr);
    strncpy(ch4[i], chaddr, 15);
    //zhuan huan cheng uint32_t
    daddr4[i] = inet_addr(ch4[i]);
    //ipv4 yan ma chang du
    fscanf(fp,"%s",chaddr);
    strncpy(fix4[i], chaddr, 5);  
    //qiu wang luo di zhi he guang bo di zhi
    daddr4[i]=fanhuiwldz(daddr4[i],fix4[i]); 
    daddr4gb[i]=fanhuigbdz(daddr4[i],fix4[i]);    
    //tong guo yi wei jiang di zhi dao guo lai
    daddr4[i]=zhuanhuan(daddr4[i]); 
    daddr4gb[i]=zhuanhuan(daddr4gb[i]);  
  
    logmsg(ANDROID_LOG_FATAL, "%s  %s  %s  %s  %x  %x",ch6[i],fix6[i],ch4[i],fix4[i],daddr4[i],daddr4gb[i]);
  //   logmsg(ANDROID_LOG_FATAL, "%s  %s  %s  %s  %x",ch6[i],fix6[i],ch4[i],fix4[i],daddr4[i]);
   }
   //uint32_t addr1 = inet_addr("180.76.03.151");
   //logmsg(ANDROID_LOG_FATAL,"%x",addr1);
   //uint32_t addr2 = inet_addr("180.076.003.151");
   //logmsg(ANDROID_LOG_FATAL,"%x",addr2);
   //uint32_t addr3 = inet_addr("119.75.217.56");
   //logmsg(ANDROID_LOG_FATAL,"%x",addr3);
   uint32_t addr=zhuanhuan(inet_addr("180.76.3.151"));
   unsigned long addrlong=zhuanhuan(inet_addr("180.76.3.151"));
   unsigned long addrlongwldz=zhuanhuan(inet_addr("180.76.3.0"));
   unsigned long addrlonggbdz=zhuanhuan(inet_addr("180.76.3.255"));
   unsigned long addrzh=addr;
   //logmsg(ANDROID_LOG_FATAL,"%x,%lu,%lu,%lu,%lu",addr,addrlong,addrlongwldz,addrlonggbdz,addrzh);
  // if(addr==0x97034cb4)
  //   logmsg(ANDROID_LOG_FATAL,"addr shi 97034c");
  // if(addr>0x00034cb4)
  //   logmsg(ANDROID_LOG_FATAL,"addr da yu 34cb4");
  uint32_t shuchu = zhuanhuan(addr);
  //logmsg(ANDROID_LOG_FATAL,"%x",shuchu);
}
//zi ji jia..
//qiu wang luo di zhi
uint32_t fanhuiwldz(uint32_t daddr4,char *fix4){
  uint32_t wldz=0;
  uint32_t yanma=fanhuiyanma(fix4);
  wldz= daddr4 & yanma;
  return wldz;
}
//qiu guang bo di zhi
...
uint32_t fanhuiyanma(char *fix4){
  uint32_t yanma=0;
  if(strcmp(fix4,"8")==0){
     yanma=inet_addr("255.0.0.0");
     return yanma;
  }
  if(strcmp(fix4,"9")==0){
     yanma=inet_addr("255.128.0.0");
     return yanma;
  }
  if(strcmp(fix4,"10")==0){
     yanma=inet_addr("255.192.0.0");
     return yanma;
  }
  if(strcmp(fix4,"11")==0){
     yanma=inet_addr("255.224.0.0");
     return yanma;
  }
  if(strcmp(fix4,"12")==0){
     yanma=inet_addr("255.240.0.0");
     return yanma;
  }
  if(strcmp(fix4,"13")==0){
     yanma=inet_addr("255.248.0.0");
     return yanma;
  }
  if(strcmp(fix4,"14")==0){
     yanma=inet_addr("255.252.0.0");
     return yanma;
  }
  if(strcmp(fix4,"15")==0){
     yanma=inet_addr("255.254.0.0");
     return yanma;
  }  
  if(strcmp(fix4,"16")==0){
     yanma=inet_addr("255.255.0.0");
     return yanma;
  } 
  if(strcmp(fix4,"17")==0){
     yanma=inet_addr("255.255.128.0");
     return yanma;
  }
  if(strcmp(fix4,"18")==0){
     yanma=inet_addr("255.255.192.0");
     return yanma;
  }
  if(strcmp(fix4,"19")==0){
     yanma=inet_addr("255.255.224.0");
     return yanma;
  }
  if(strcmp(fix4,"20")==0){
     yanma=inet_addr("255.255.240.0");
     return yanma;
  }
  if(strcmp(fix4,"21")==0){
     yanma=inet_addr("255.255.248.0");
     return yanma;
  }
  if(strcmp(fix4,"22")==0){
     yanma=inet_addr("255.255.252.0");
     return yanma;
  }
  if(strcmp(fix4,"23")==0){
     yanma=inet_addr("255.255.254.0");
     return yanma;
  }
  if(strcmp(fix4,"24")==0){
     yanma=inet_addr("255.255.255.0");
     return yanma;
  }
  if(strcmp(fix4,"25")==0){
     yanma=inet_addr("255.255.255.128");
     return yanma;
  }
  if(strcmp(fix4,"26")==0){
     yanma=inet_addr("255.255.255.192");
     return yanma;
  }
  if(strcmp(fix4,"27")==0){
     yanma=inet_addr("255.255.255.224");
     return yanma;
  }
  if(strcmp(fix4,"28")==0){
     yanma=inet_addr("255.255.255.240");
     return yanma;
  }
  if(strcmp(fix4,"29")==0){
     yanma=inet_addr("255.255.255.248");
     return yanma;
  }
  if(strcmp(fix4,"30")==0){
     yanma=inet_addr("255.255.255.252");
     return yanma;
  }
  if(strcmp(fix4,"31")==0){
     yanma=inet_addr("255.255.255.254");
     return yanma;
  }
  if(strcmp(fix4,"32")==0){
     yanma=inet_addr("255.255.255.255");
     return yanma;
  }
  return yanma;
}
//zi ji jia..
//jiang di zhi dao guo lai
uint32_t zhuanhuan(uint32_t addr){
  uint32_t fhaddr=0;
  uint32_t fhaddr0,fhaddr1,fhaddr2,fhaddr3;
  fhaddr0=(addr & 0x000000ff) << 24;
  fhaddr1=(addr & 0x0000ff00) << 8;
  fhaddr2=(addr & 0x00ff0000) >> 8;
  fhaddr3=(addr & 0xff000000) >> 24;
  fhaddr=fhaddr0+fhaddr1+fhaddr2+fhaddr3;
  return fhaddr;  
}
uint16_t zhuanhuan16 (uint16_t addr){
  uint16_t fhaddr=0;
  uint16_t fhaddr0,fhaddr1;
  fhaddr0=(addr & 0x00ff) << 8;
  fhaddr1=(addr & 0xff00) >> 8;
  fhaddr=fhaddr0+fhaddr1;
  return fhaddr;
}
